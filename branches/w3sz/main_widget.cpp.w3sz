...
void Main_Widget::loadSettings()
{
	...
	// Open Hardware FIFO--added by w3sz

    hw_File = fopen (HW_FILE,"r+");
	if ( hw_File == NULL ) {
			perror( HW_FILE );
		
	                printf( "::: Unable to findhw_commands file.\n");
	   }
	//
	...
	stationCallsign = settings.readEntry (
	                      "/sdr-shell/stationCallsign", "W3SZ" );
	stationQTH = settings.readEntry (
	                 "/sdr-shell/stationQTH", "FN20ag" );
	...
	my_lat = settings.readEntry (
	             "/sdr-shell/my_lat", "40.267" ).toDouble();
	my_lon = settings.readEntry (
	             "/sdr-shell/my_lon", "75.966" ).toDouble();
	...
	AM_filter_l = settings.readEntry (
	                  "/sdr-shell/AM_filter_l", "-4000" ).toInt();
	AM_filter_h = settings.readEntry (
	                  "/sdr-shell/AM_filter_h", "4000" ).toInt();
	//added by w3sz
	preampVal = 	settings.readEntry (
					  "/sdr-shell/preampVal","1").toInt();
			fprintf( hw_File, "setPreampGain %d\n", preampVal);
			fflush( hw_File ); 
	
	//added by w3sz [LT2208 dither and random both fixed to 'on']
		fprintf( hw_File, "setLT2208Dither %d\n", 1);
   
        fprintf( hw_File, "setLT2208Random %d\n", 1);
	//
	useHamlib = ( bool ) settings.readEntry (
	            "/sdr-shell/useHamlib", "0" ).toInt();
	rigString = settings.readEntry (
	                "/sdr-shell/hamlib_rig", "1" );
	rig = rigString.toInt();
...
void Main_Widget::saveSettings()
{
	...
	//These are saved to /root/.qt/sdr-shellrc
	...
	// Save preamp values
	settings.writeEntry ("/sdr-shell/preampVal",preampVal);
	...
	switch ( e->state() )
	{
		...
		case Qt::AltButton:
			switch ( e->key() )
			{
				...
				case 72: setCfg(4); break; //h calls up help
				default: break;
			}
			break;
		default:
...
void Main_Widget::rx_cmd ( int key ) // Leave for IF shift now.
{
	...
	switch ( key )
	{
		...
		case 71: //g
			...
			break;
		//added by w3sz
		case 44: //comma, used for preamp gain down
			fprintf( hw_File, "setPreampGain %d\n", 0);
			fflush( hw_File ); 			
			preampVal = 0;			
			break;
		case 46: //period, used for preamp gain up
			fprintf( hw_File, "setPreampGain %d\n", 1);
			fflush( hw_File ); 	
			preampVal = 1;
			break;
...
void Main_Widget::setDefaultRxFrequency ( )
{
	switch ( mode )
	{
		...
		//	rx_delta_f = rx_if - rx_f + slopeTuneOffset;
			rx_delta_f = slopeTuneOffset + 11000 ;
			break;
	}
	setRxFrequency();
}
...
void Main_Widget::setRxFrequency()
{
	char text[20];
	if ( !useIF )
	//frequency adjusted by w3sz
	{
		sprintf ( text, "%11.6lf", ( double ) ( rx_f - rx_delta_f + 11000 ) / 1000000.0 );
		lcd->display ( text );
	}
...
//  added by w3sz  
    fprintf( hw_File, "setFrequency %f\n", (rx_f - rx_delta_f) / 1000000.0 );
    fflush( hw_File ); 
//
}
